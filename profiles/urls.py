...
urlpatterns = [path('list/', ProfileList.as_view(), name="profiles_list"),
               ...
               path('change-password/<int:toto>/',
                    PasswordChangeView.as_view(success_url=reverse_lazy('profiles:password_change_done')),
                    name="change-password"),
               path('change-password-done/',
                    PasswordChangeDoneView.as_view(),
                    name="password_change_done"),
               path('reset-password/',
                    PasswordResetView.as_view(success_url=reverse_lazy('profiles:password_reset_done')),
                    name="password_reset"),
               path('reset-password-done/',
                    PasswordResetDoneView.as_view(),
                    name="password_reset_done"),
               path('reset-password-confirm/<uidb64>/<token>/',
                    PasswordResetConfirmView.as_view(success_url=reverse_lazy('profiles:password_reset_complete')),
